...
class GstAVPipeline:
    def __init__(
        ...
        self._alsasrc: Optional[Gst.Element] = None
        self._queue_audio: Optional[Gst.Element] = None
    ...
    def _webrtcsrc_pad_added_cb(self, webrtcsrc: Gst.Element, pad: Gst.Pad) -> None:
        if pad is not None and pad.get_name().startswith("audio"):  # type: ignore[union-attr]
            ...
            if self._alsasrc is not None and self._queue_audio is not None and self._aec != "off":
    ...
    async def _removing_webrtcsrc(self) -> None:
        if self._pipeline is not None:
            async with self._mutex:
                ...
                if self._webrtcdsp:
                    assert self._alsasrc is not None
                    assert self._queue_audio is not None
